...
def gen_dim_calculator(dim_info: DimInfo, is_ptr: bool) -> str:
    ...
    if dim_info.source == Source.INPUT:
        if   dim_info.tensor_idx == 0:
            prefix += "a_dim"
        elif dim_info.tensor_idx == 1:
            prefix += "b_dim"
        elif dim_info.tensor_idx == 2:
            prefix += "m_dim"        # ← new metadata case
        else:
            raise RuntimeError(f"Unsupported input tensor idx {dim_info.tensor_idx}")
    else:
        assert (
